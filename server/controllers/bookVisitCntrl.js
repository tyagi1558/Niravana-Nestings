...
export const exportBookVisitToCSV = asyncHandler(async (req, res) => {
  try {
    const bookVisits = await prisma.BookVisit.findMany({
      include: {
        Residency: {
    ...
    const csvWriter = createObjectCsvWriter({
      ...
      header: [
        ...
        { id: "nextFollowUp", title: "Next Follow Up" },
        { id: "calendarCondition", title: "Calendar Condition" },
        { id: "property", title: "Property" },
        { id: "propertyId", title: "Property ID" },
      ],
    });

    // Write formatted bookVisits data to CSV file
    await csvWriter.writeRecords(formattedBookVisits);

    const fileUrl = `${req.protocol}://${req.get("host")}/api/bookVisit/bookVisits.csv`;

    res.send({ url: fileUrl }); // Return the URL of the CSV file
  } catch (err) {
    throw new Error(err.message);
  }
});